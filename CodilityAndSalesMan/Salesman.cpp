...
int deplacement_salesman(int pos, int visited, const vector<vector<int>>& dist, vector<vector<int>>& memo) 
{
    ...
    for (int next = 0; next < dist.size(); ++next) 
    {
        ...
        {  
            ...
            if (new_res < 1e6) 
            {
                new_res += deplacement_salesman(next, visited | (1 << next), dist, memo);
                res = min(res, new_res);
            }
        }
    }
...
int salesman() 
{
    ...
    int result = deplacement_salesman(0, 1, dist, memo);
